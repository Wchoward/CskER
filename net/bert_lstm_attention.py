...
class BertForSequenceClassificationLSTMAttenion(BertPreTrainedModel):
    ...
    def forward(
        ...
        sorted_batch, sorted_lengths, _, restoration_idx = sort_by_seq_lens(self.args, hidden_output, sequences_lengths)
        ...
        last_attention = self.attention(reordered_outputs, attention_mask[:, :batch_max_length])
...
class BertForSequenceClassificationCskLSTMAttenion(BertPreTrainedModel):
    def __init__(self, config, args):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.args = args
        ...
        self.lstm = nn.LSTM(
            (config.hidden_size + args.num_trans_units * 2),
            (config.hidden_size + args.num_trans_units * 2),
            num_layers=1,
            bias=True,
            batch_first=True,
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.nums = 2
        hidden_size_changed = config.hidden_size + (config.hidden_size + args.num_trans_units * 2) * self.nums
        self.pooler = nn.Sequential(nn.Linear(hidden_size_changed, config.hidden_size), nn.Tanh())
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        # if args.fl_gamma:
        #     self.label_focal_loss = FocalLoss(num_class=config.num_labels,
...
class Attention(nn.Module):
    def __init__(self, config, args):
        super().__init__()
        self.attn_weight = nn.Linear((config.hidden_size + args.num_trans_units * 2) * 2, 1, bias=False)
        self.head_num = 1
